...
describe('<ParserComponent /> Unit Tests', () => {
  ...
  describe('Upload small csv file', () => {
    const props = {
      prompt: 'test size file: ',
      fileType: FileType.csv,
    };
    ...
    it('Onchange event triggered when file selected', async () => {
      ...
      if (size < 100) {
        expect(onChangeMock).toHaveBeenCalledTimes(1);
        expect(testFile.name).toBe('testFile.name');
        expect(testFile.size).toBe(109);
      }
    });
  });
});
