...
describe('Timeline is correctly resized when user uses ' +
  ...
  it('Initialize snapshot', ()=>{
    const tree = renderer
        .create(<TimelineComponent data={data}/>)
        .toJSON();
    expect(tree).toMatchSnapshot();
  });


  it('Timeline is correctly resized when zoom in by mouse wheel', () => {
    const testSnap = renderer.create(<TimelineComponent data={data}/>);
  });
});
