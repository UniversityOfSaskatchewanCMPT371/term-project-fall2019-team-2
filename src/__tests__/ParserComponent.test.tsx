...
describe('Csv FileEvents processed correctly', () => {
  ...
  it('should thrown an error if a CSV file parsed in has ' +
    'no headers', async () => {
    const onChangeMock = jest.fn();
    const testFile: File = new File(
        [
    ...
    console.log(await instance.parse(event));

    /*
     expect(await instance.parse(event)).toThrow('header');
    */
  });
});
